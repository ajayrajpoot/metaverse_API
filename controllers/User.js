...
exports.signup = async (req, res, next) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'User created', userId: result, Result: true });
    } catch (error) {
...
exports.get_otp = async (req, res, next) => {
    ...
    try {
        ...
        let result = await writeDB.query(`UPDATE users SET otp = ${code}   WHERE (email='${username}' or username='${username}' or phone_no='${username}');`);
        ...
        // otpcode.add( {"code":code,"id":result[0].id} )
        res.status(200).json({ Message: `OTP  ${code} for Reset.`, Result: true });
...
exports.reset_password = async (req, res, next) => {
    try {
        ...
        if (result.affectedRows > 0) {
            res.status(200).json({ Message: `  Reset Password.`, Result: true });
        } else {
            res.status(200).json({ Message: `Invaild OTP.`, Result: false });
        }
